...
8. 自动生成调整计划和下一轮参数草案
9. 自动运行一轮线性迭代并比较结果
10. 在网页工作台中播放、查看、循环和导出
11. 导出 `MIDI` 和 `MusicXML`
...
每次运行 `iterate.py` 后，系统会自动完成：
- 读取当前 `latestVersionId`
- 确保当前版本已有调整计划和下一轮参数草案
- 用新参数重跑分析器
- 生成下一版本 `candidate.json`
- 生成下一版本 `evaluation-report.json`
- 生成下一版本 `params.json`
- 导出下一版本 `export.mid` / `export.musicxml`
- 生成版本比较文件
- 更新 `task.json` 中的版本索引和 best/latest 指针
...
- `analyzer/`：Python 分析脚本，负责提音、分离、分析、评测、归档、调参、迭代、导出
- `web/`：Next.js 前端原型
- `samples/`：样本说明
- `samples/raw/`：原始视频或混合音频输入
- `output/`：提取音频、分离结果、分析 JSON、评测报告、任务归档、调整计划、比较文件、导出文件
- `docs/`：产品、方法论和开发拆解文档
...
- 初始分析结果：`output/analysis/test1.analysis.json`
- 初始评测报告：`output/tasks/task_other/versions/ver_001/evaluation-report.json`
- 初始调整计划：`output/tasks/task_other/versions/ver_001/adjustment-plan.json`
- 初始下一轮参数：`output/tasks/task_other/versions/ver_001/next-params.json`
